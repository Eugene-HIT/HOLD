...
# Replace EVERYTHING starting from the FIRST private fun streamPcmToEsp32 
# up to the LAST occurrence before private fun processAndUploadAudio
content = re.sub(r'(    private fun streamPcmToEsp32[\s\S]*?)(\s+private fun processAndUploadAudio)', r'\2', content)

# Okay, now the file DOES NOT have streamPcmToEsp32, startPttRecording, stopPttRecordingAndSend.
# We will inject them right before processAndUploadAudio.
...
extract_stream = '''
    ...
    private fun startPttRecording() {
        ...
        val minBufSize = android.media.AudioRecord.getMinBufferSize(22050, android.media.AudioFormat.CHANNEL_IN_MONO, android.media.AudioFormat.ENCODING_PCM_16BIT)
        pttAudioRecord = android.media.AudioRecord(android.media.MediaRecorder.AudioSource.MIC, 22050, android.media.AudioFormat.CHANNEL_IN_MONO, android.media.AudioFormat.ENCODING_PCM_16BIT, minBufSize * 2)
