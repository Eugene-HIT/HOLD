...
"""
创建时间: 2026-06-23
文件主要职责: 采集 MAX30102 手指模式 VOFA 串口输出，并导出带表头 CSV。
核心输入输出:
- 输入: 串口号、波特率、采集时长；默认适配 COM9 / 115200 / 60 秒。
- 输出: 单份 CSV，包含当前 MAX30102 VOFA 运行帧的 9 列数据。
最后更改时间: 2026-06-23
累加式更改日志:
- 2026-06-23: 新增首版 MAX30102 手指模式采集脚本。
注意事项:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Capture MAX30102 finger-mode VOFA serial stream and export CSV."
    )
    ...
    parser.add_argument(
        "--output-root",
        default="captures/max30102_finger",
        help="Root output directory, default: captures/max30102_finger",
    )
...
def main() -> int:
    ...
    csv_path = output_dir / "max30102_finger_vofa.csv"
