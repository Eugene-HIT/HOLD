...
async function getLlmReply(event) {
  ...
  if (!apiKey || !baseUrl) {
    return '已收到设备数据，云端基础链路已通。';
  }

  const prompt = `收到来自 ${event.device_id} 的 ${event.event_type} 数据，请返回一句 20 字以内的确认文本，说明蓝牙、云端和模型链路已通。`;
...
exports.main = async (event) => {
  ...
  const sanitizedEvent = {
    ...
    respiration_bpm: Number(event.respiration_bpm || 0),
    heart_rate_bpm: Number(event.heart_rate_bpm || 0),
    body_temp_c: Number(event.body_temp_c || 0),
    pressure_raw: Number(event.pressure_raw || 0),
    pressure_level: Number(event.pressure_level || 0),
    ppg_ir: Number(event.ppg_ir || 0),
    ppg_red: Number(event.ppg_red || 0),
    wearing: Boolean(event.wearing),
    contact_present: Boolean(event.contact_present),
